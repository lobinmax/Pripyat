...
Public Class frAddNewPr_Books_Judges
    Dim DG_Judges_row As Object = frBooksJudicialArea.DG_Judges.CurrentRow     ' Активная строка на гриде с Судебными участками
    ...
    Private Sub frAddNewPr_Books_Judges_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If AddOrEdit = 3 Then
            Me.Text = "Изменение данных судьи"
            ' Заполнение полей данными из активной строки на DG_JudicialArea
            Me.cmb_CourtType.SelectedValue = DG_Judges_row.Cells("CourtTypeId").Value.ToString()
            Me.txt_JudgesName.Text = DG_Judges_row.Cells("Name").Value.ToString()
            ' Если данные из базы уже есть то, меняем настройки маски
            If DG_Judges_row.Cells("Phone").Value.ToString() <> "" Then
                Me.txt_Phone.TextMaskFormat = MaskFormat.IncludePromptAndLiterals
                Me.txt_Phone.Text = DG_Judges_row.Cells("Phone").Value.ToString()
            End If
            ' Если данные из базы уже есть то, меняем настройки маски
            If DG_Judges_row.Cells("PhoneMobile").Value.ToString() <> "" Then
                Me.txtM_PhoneMobile.TextMaskFormat = MaskFormat.IncludePromptAndLiterals
                Me.txtM_PhoneMobile.Text = DG_Judges_row.Cells("PhoneMobile").Value.ToString()
            End If
            ' Если данные из базы уже есть то, меняем цвет текста и маску
            If DG_Judges_row.Cells("email").Value.ToString() <> "" Then
                Me.txtM_mail.Mask = ""
                Me.txtM_mail.ForeColor = WindowText
                Me.txtM_mail.Text = DG_Judges_row.Cells("email").Value.ToString()
            End If
...
#End Region
            ...
            If AddOrEdit = 2 Then ' Добавление записи
                If ExecuteQuery(
                                                "EXEC Pr_Books_Judges " & _
                                                    ...
                                                    "@Name = " & ConvertToNull(Me.txt_JudgesName.Text, True, 0) & ", " & _
                                                    ...
                                                    "@Function = " & AddOrEdit,
 _
                                                "AddNew.Pr_Books_Judges"
                                             ) Then
                    With New frInfo     ' Всплываюшее сообщение 
                        .Mess = "В справочник добавлен новый судья..."
                        .Show()
                    End With
            ...
            If AddOrEdit = 3 Then   ' Изменение записи
                If ExecuteQuery(
                                                "EXEC Pr_Books_Judges " & _
                                                    "@JudgeId = " & DG_Judges_row.Cells("JudgeId").Value.ToString() & ", " & _
                                                    "@CourtTypeId = " & Me.cmb_CourtType.SelectedValue & ", " & _
                                                    "@Name = " & ConvertToNull(Me.txt_JudgesName.Text, True, 0) & ", " & _
                                                    "@Phone = " & ConvertToNull(Me.txt_Phone.Text, True, 0) & ", " & _
                                                    "@PhoneMobile = " & ConvertToNull(Me.txtM_PhoneMobile.Text, True, 0) & ", " & _
 ...
 _
    ...
    Private Function CheckFill() As Boolean
        If Me.txt_JudgesName.Text = "" _
        Or Me.cmb_CourtType.Text = "" Then
